...
% the files should have a single subj_data table variable containing one
...
patients = string({patients_dir(~cat(1,patients_dir.isdir)).name});
...
for pat = 1:length(patients)
    ...
    Sz_preproc = {};
    load(sprintf('%s/%s.mat', data_location, patient), 'Sz_preproc');
    if isempty(Sz_preproc)
        continue
    end
    %% calculate imprint
    mad_thresh = 5;
    ...
    lambda = 200;
    prop_rec = 0.7;
    [data_tbl, cell_imprint_mahal] = calc_imprint_mahal_page_hinkley(Sz_preproc,...
        'window_size', 1, ...
        'window_overlap', 7/8, ...
    ...
    save("../SWEZ_data/Imprints/"+patient,"data_tbl","imprints");
end